...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h = np.maximum(0, W1.dot(X.T) + b1[:,np.newaxis])
    scores = (W2.dot(h) + b2[:,np.newaxis]).T
    ...
    esum = np.sum(np.exp(scores), axis=1)
    l_softmax = np.sum(-np.log( np.exp(scores[np.arange(N),y]) / esum))
    l_regularization = 0.5*reg*(np.sum(W1*W1)+np.sum(W2*W2))
    loss = l_softmax/N + l_regularization
	...
	# ================================================================ #
    p = np.exp(scores) / esum[:,np.newaxis]
    p[np.arange(N),y]-=1
    p/=N # Normalization

    grads['W2'] = reg*W2 + p.T.dot(h.T)
    grads['b2'] = np.sum(p, axis=0)

    grads['W1'] = reg*W1 + ((W1.dot(X.T) > 0)*W2.T.dot(p.T)).dot(X)
    grads['b1'] = np.sum((W1.dot(X.T) > 0)*W2.T.dot(p.T), axis=1)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # YOUR CODE HERE:
  	  ...
  	  # ================================================================ #
      samples = np.random.choice(np.arange(X.shape[0]), batch_size)
      X_batch = X[samples]
      y_batch = y[samples]
