...
def affine_forward(x, w, b):
    ...
    #   assignments.
    # ================================================================ #
    newshape = (x.shape[0], np.prod(x.shape[1:]))
    out = np.reshape(x,newshape).dot(w) + b[np.newaxis,:]
...
def affine_backward(dout, cache):
    ...
    dx = np.reshape(dout.dot(w.T), x.shape) # same as x
    dw = (np.reshape(x, (N,D)).T).dot(dout)# DxN NxM => DxM
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_hat = (x-sample_mean[np.newaxis,:]) / np.sqrt(sample_var[np.newaxis,:] + eps)
        ...
        x_hat = (x-running_mean[np.newaxis,:]) / np.sqrt(running_var[np.newaxis,:] + eps)
...
def batchnorm_backward(dout, cache):
    ...
    # ================================================================ #
    x, gamma, x_hat, sample_var, sample_mean, eps = cache
    m = x.shape[0]
    ...
    de = -.5*1.0/np.power(sample_var+eps, 1.5)*(x - sample_mean)*dx_hat
    dvar = np.sum(de, axis=0)
    dmean = -1 / (np.sqrt(sample_var + eps)) * np.sum(dx_hat, axis=0)
    dx = da + 2*(x-sample_mean)/m*dvar + 1/m*dmean
