...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
      ...
      test_score = scores[y[i]]
      for j in range(num_classes):
        if j != y[i]:
          loss += np.maximum(0, 1 + scores[j] - test_score)
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
      ...
      for j in range(num_classes):
        if j != y[i]:
          delta_l = np.maximum(0, 1 + scores[j] - test_score)
          if delta_l != 0:
            grad[j] += X[i].T
            grad[y[i]] -= X[i].T
          loss += delta_l
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    delta_l = np.maximum(0, 1 + scores-scores[y,np.arange(num_train)])
    delta_l[y, np.arange(num_train)] = 0

    loss = np.sum(delta_l) / num_train
    ...
    # ================================================================ #
    indicator = delta_l
    indicator[delta_l > 0] = 1

    indicator_y = np.zeros(delta_l.shape)
    indicator_y[y, range(num_train)] = np.sum(indicator, axis=0)
    delta_grad = (indicator.dot(X) - indicator_y.dot(X))

    grad = delta_grad/num_train
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
