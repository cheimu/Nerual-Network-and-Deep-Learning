...
def rmsprop(w, dw, config=None):
  ...
  # ================================================================ #
  config['a'] = config['decay_rate'] * config['a'] + (1 - config['decay_rate']) * np.square(dw)
...
def adam(w, dw, config=None):
  ...
  config['a'] = config['beta2'] * config['a'] + (1 - config['beta2']) * np.square(dw)

  v_tilde = 1 / (1 - np.power(config['beta1'], config['t'])) * config['v']
  a_tilde = 1 / (1 - np.power(config['beta2'], config['t'])) * config['a']
  next_w = w - config['learning_rate'] /(np.sqrt(a_tilde)+ config['epsilon']) * v_tilde 
